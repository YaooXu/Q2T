...
def _set_cache_path(path_template, dataset, alias, model):
    if path_template is not None:
        cache_path = path_template.format(dataset=dataset, model=model, alias=alias)
...
class KBCEngine(object):
    def __init__(self, opt):
        ...
        self.cache_eval = _set_cache_path(opt['cache_eval'], opt['dataset'], self.alias, opt['model'])
        self.model_cache_path = _set_cache_path(opt['model_cache_path'], opt['dataset'], self.alias, opt['model'])
        opt['model_cache_path'] = self.model_cache_path
        ...
        self.optimizer = setup_optimizer(self.model, opt['optimizer'], opt['learning_rate'], opt['decay1'],
                                         opt['decay2'])
            ...
            while exp_train_sampler.is_epoch(e):  # iterate through all batchs inside an epoch
                ...
                if self.world == 'LCWA':
                    input_batch_train = exp_train_sampler.batchify(self.batch_size,
                                                                   self.device)
                    predictions, factors = self.model.forward(input_batch_train, score_rel=self.score_rel,
                                                              score_rhs=self.score_rhs, score_lhs=self.score_lhs)
                    ...
                    if self.score_rel and self.score_rhs and self.score_lhs:
                        ...
                        # print('----2----')
                        l_fit = self.loss(predictions[0], input_batch_train[:, 2]) + self.w_rel * self.loss(
                            predictions[1], input_batch_train[:, 1])
                    elif self.score_lhs and self.score_rel:
                        # print('----3----')
                        pass
                    elif self.score_rhs and self.score_lhs:  # standard
                        # print('----4----')
                        l_fit = self.loss(predictions[0], input_batch_train[:, 2]) + self.loss(predictions[1],
                                                                                               input_batch_train[:, 0])
                    ...
                    l_reg, l_reg_raw, avg_lmbda = self.regularizer.penalty(input_batch_train,
                                                                           factors)  # Note: this shouldn't be included into the computational graph of lambda update
                ...
                if ((step_idx % 1000 == 0 and step_idx > 1000) or (
                        step_idx <= 1000 and step_idx % 100 == 0)):  # reduce logging frequency to accelerate
            ...
            if e % self.opt['valid'] == 0:
                ...
                for split in self.dataset.splits:
                    res_s = self.dataset.eval(model=self.model,
                                              split=split,
                                              n_queries=-1 if split != 'train' else 1000,
                                              # subsample 5000 triples for computing approximated training MRR
                                              n_epochs=e)
