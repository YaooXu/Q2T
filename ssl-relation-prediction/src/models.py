        ...
        with torch.no_grad():
            ...
            while c_begin < self.sizes[2]:
                ...
                while b_begin < len(queries):
                    ...
                    for i, query in enumerate(these_queries):
                        existing_s = (query[0].item(),
                                      query[1].item()) in filters  # reciprocal training always has candidates = rhs
                        existing_r = (query[2].item(), query[1].item() + self.sizes[
                            1]) in filters  # standard training separate rhs and lhs
                        if candidates == 'rhs':
                            if existing_s:
                                filter_out = filters[(query[0].item(), query[1].item())]
                                filter_out += [queries[b_begin + i, 2].item()]
...
class TuckER(KBCModel):
    def __init__(self, sizes, rank_e, rank_r, init_size=1e-3, dp=0.5):
        ...
        self.entity = nn.Embedding(sizes[0], rank_e)
        self.relation = nn.Embedding(sizes[1], rank_r)
    ...
    def get_queries(self, queries, target='rhs'):
        ...
        if target == 'rhs':
            lhs_proj = torch.matmul(self.core.data.transpose(0, 2), lhs.transpose(0, 1)).transpose(0,
                                                                                                   2)  # b, rank_r, rank_e
            rel_proj = rel.view(-1, 1, self.rank_r)
...
class CP(KBCModel):
    ...
    def param_grad(self):
        return (
            self.lhs.weight.grad.data.detach(), self.rel.weight.grad.data.detach(), self.rhs.weight.grad.data.detach())
...
class DistMult(KBCModel):
    ...
    def forward(self, x, score_rhs=True, score_rel=False, score_lhs=False, normalize_rel=False):
        ...
        rhs_scores, rel_scores = None, None
        if score_rhs:
            rhs_scores = (lhs * rel) @ self.entity.weight.t()
        if score_rel:
            rel_scores = (lhs * rhs) @ self.relation.weight.t()
        if score_lhs:
            lhs_scores = (rhs * rel) @ self.entity.weight.t()
    ...
    def get_queries(self, queries, target='rhs'):
        if target == 'rhs':
            return self.entity(queries[:, 0]).data * self.relation(queries[:, 1]).data
        elif target == 'lhs':
            return self.entity(queries[:, 2]).data * self.relation(queries[:, 1]).data
        elif target == 'rel':
            return self.entity(queries[:, 0]).data * self.entity(queries[:, 2]).data
